...
result_dir = f"{model_dir}/conv_edge_NOT/{reduce_mode}_{trace_name}_attack_overlap"
...
def filter_edge_by_key(trace: AttrMap,
                    key: str =TraceKey.EDGE,
                    graph = LENET.network_class.graph().load(),
    ):
    reconstruct_edge_fn = partial(reconstruct_edge,
                                trace,
                                graph,
                                key,
                                )
...
def reduce_edge_mask(edge_mask: AttrMap, reduce_mode="none"):
    ...
    for node_name in edge_mask:
        ...
        if "conv2d" in node_name:
            if reduce_mode == "feature":
                edge_sum = edge_mask[node_name].sum(-1).sum(-1)
                edge_sum[edge_sum>0] = 1
            elif reduce_mode == "kernel":
                edge_sum = edge_mask[node_name].sum(1).sum(1)
                edge_sum[edge_sum>0] = 1
            elif reduce_mode == "channel":
                edge_sum = edge_mask[node_name].sum(0).sum(2)
                edge_sum[edge_sum>0] = 1
            elif reduce_mode == "output":
                edge_sum = edge_mask[node_name].sum(-1).sum(-1).sum(-1)
                edge_sum[edge_sum>0] = 1
            elif reduce_mode == "space":
                edge_sum = edge_mask[node_name].sum(1).sum(1).sum(-1).sum(-1)
                edge_sum[edge_sum>0] = 1
            elif reduce_mode == "except_space":
                edge_sum = edge_mask[node_name].sum(0).sum(0).sum(0)
                edge_sum[edge_sum>0] = 1
            else:
...
def detect_by_reduced_edge(class_trace, trace, reduce_mode = "none"):
    class_masks = filter_edge_by_key(class_trace)
    sample_masks = filter_edge_by_key(trace)
...
# Count how many neurons violate zero constrain under current reduce mode

def detect_by_reduced_edge_count_violation(class_trace, trace, reduce_mode = "none"):
    class_masks = filter_edge_by_key(class_trace)
    sample_masks = filter_edge_by_key(trace)
    class_masks = reduce_edge_mask(class_masks, reduce_mode = reduce_mode)
    sample_masks = reduce_edge_mask(sample_masks, reduce_mode = reduce_mode)
    ...
    for node_name in class_masks:
        ...
        sample_mask = sample_masks[node_name]
        class_zero = class_mask==0
        class_one = class_mask>0
        sample_one = sample_mask>0
        sample_zero_sum = sample_mask[class_zero].sum()
        violation[node_name] = sample_zero_sum
        violation[f"{node_name}_zero"] = class_zero.sum()
        violation[f"{node_name}_one"] = class_one.sum()
        violation[f"{node_name}_sample_one"] = sample_one.sum()
        if sample_zero_sum>0:
            is_adversarial = True
    if is_adversarial:
        return adversarial_label, violation
    else:
        return normal_label, violation
...
# Compute the mean overlap ratio of attacked image
def attack_reduced_edge_detection_count_violation(
    attack_name: str,
    attack_fn,
    generate_adversarial_fn,
    class_trace_fn: Callable[[int], IOAction[AttrMap]],
        ...
        def get_row(class_id: int, image_id: int) -> Dict[str, Any]:
            ...
            row = {
                ...
                "original.prediction":
                    original_pred,
                "adversarial.prediction":
                    detect_by_reduced_edge(class_trace_fn(adversarial_label).load(),
                                            adversarial_trace,
                                            reduce_mode,
                                            ),
                "violation": violation,
...
def draw_violations():
    dir = os.path.join(result_dir, "violations/original")
    os.makedirs(dir, exist_ok=True)

    # ops include conv2d/Conv2D, conv2d_1/Conv2D, dense/MatMul, dense_1/MatMul,
    # dense_2/MatMul
    for reduce_mode in mode_to_violation:
        labels = []
        hists = []
        for op in [
            "conv2d/Conv2D",
            ...
            violation_cnt = [v[op] for v in mode_to_violation[reduce_mode]]
            violation_cnt = [v for v in violation_cnt if v > 0 ]
            zero_cnt = mode_to_violation[reduce_mode][0][f"{op}_zero"]
            one_cnt = mode_to_violation[reduce_mode][0][f"{op}_one"]
            label = f"{op}_{zero_cnt}_{one_cnt}"
            labels.append(label)
            hists.append(violation_cnt)
        bins = max(max(hists))
        bins = min(bins, 20)
        plt.hist(hists, bins = bins, label=labels)
        plt.title(reduce_mode)
        plt.legend()
        plt.xlabel("Violation number in the reduced edge")
        plt.ylabel("Count")
        plt.savefig(f"{dir}/{reduce_mode}_conv.png")
        plt.clf()
        ...
        plt.xlabel("Violation number in the reduced edge")
        plt.ylabel("Count")
        plt.savefig(f"{dir}/{reduce_mode}_fc.png")
        plt.clf()
        ...
        for op in [
            ...
            violation_cnt = [v[f"{op}_sample_one"] for v in mode_to_violation[reduce_mode]]
            violation_cnt = [v for v in violation_cnt if v > 0 ]
            label = f"{op}"
            labels.append(label)
            hists.append(violation_cnt)
        ...
        plt.xlabel("Violation number in the reduced edge")
        plt.ylabel("Count")
        plt.savefig(f"{dir}/{reduce_mode}_conv_sample_cnt.png")
        plt.clf()
        ...
        plt.xlabel("Violation number in the reduced edge")
        plt.ylabel("Count")
        plt.savefig(f"{dir}/{reduce_mode}_fc_sample_cnt.png")
        plt.clf()

def attack_reduce_mode_exp():
    exp_to_roc = {}

    global result_dir
    result_dir = f"{model_dir}/reduce_{trace_name}_NOT"
    os.makedirs(result_dir, exist_ok=True)

    global mode_to_violation
    mode_to_violation = {}

    transforms, transform_name = None, "noop"
    # reduce_mode includes kernel, feature, channel, none
    for reduce_mode, color in [
        ["none", 'b'],
        ["kernel", 'r'],
        ["channel", 'g'],
        ["feature", 'm'],
    ]:
        exp_name = attack_name+"_"+reduce_mode
        print(exp_name)
        # print(mode_to_violation)
        trace_frame = attack_transform_overlap(attack_name,
                                            transform_name,
                                            transforms,
                                            reduce_mode = reduce_mode,
