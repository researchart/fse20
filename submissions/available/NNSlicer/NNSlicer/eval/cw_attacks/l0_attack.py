...
class CarliniL0(CWAttack):
    ...
    def gradient_descent(self, sess, model):
        ...
        init = tf.variables_initializer(
            var_list=[modifier, canchange, simg, original, timg, tlab] + new_vars
        )
    ...
    def attack_single(self, img, target):
        ...
        valid = np.ones(
            (1, self.model.image_size, self.model.image_size, self.model.num_channels)
        )
        ...
        while True:
            ...
            equal_count = self.model.image_size ** 2 - np.sum(
                np.all(np.abs(img - nimg[0]) < 0.0001, axis=2)
            )
            ...
            if self.independent_channels:
                ...
                valid = valid.reshape(
                    (self.model.image_size ** 2, self.model.num_channels)
                )
                totalchange = abs(np.sum(nimg[0] - img, axis=2)) * np.sum(
                    np.abs(gradientnorm[0]), axis=2
                )
            ...
            for e in np.argsort(totalchange):
                if np.all(valid[e]):
                    ...
                    if totalchange[e] > 0.01:
                        # if this pixel changed a lot, skip
                        break
                    if did >= 0.3 * equal_count ** 0.5:
                        # if we changed too many pixels, skip
                        break

            valid = np.reshape(
                valid, (1, self.model.image_size, self.model.image_size, -1)
            )
