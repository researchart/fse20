...
dir =  f"models/prune/f1_score/allconv_weight"
modes = [
    "posneg_small",
    "posonly_small",
]
...
def draw_posneg_weight_cmp_hist():
    ...
    for ratio in np.arange(0.4, 0.7, 0.1):
        ...
        fig_name = f"posneg_weight_cmp_hist_alltrace_ratio{round(ratio,1)}.pdf"
        fig_path = os.path.join(fig_dir, fig_name)
        plt.savefig(fig_path)
        plt.clf()
    ...
    for ratio in np.arange(0.4, 0.7, 0.1):
        ...
        fig_name = f"posneg_weight_cmp_hist_classtrace_ratio{round(ratio,1)}.pdf"
        fig_path = os.path.join(fig_dir, fig_name)
        plt.savefig(fig_path)
        plt.clf()
