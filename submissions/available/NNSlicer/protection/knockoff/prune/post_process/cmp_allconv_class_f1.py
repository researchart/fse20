...
dir =  f"models/prune/f1score_avg/allprune_classf1"
modes = [
    "posneg_small",
    "posonly_small",
]
...
def draw_posneg_weight_cmp_hist():
    ...
    all_df = load_csv(f"allconv_class_all.csv")
    
    class_df = []
    for class_id in range(10):
        df = load_csv(f"allconv_class_{class_id}.csv")
    ...
    for ratio in np.unique(all_df['ratio']):
        ...
        channel = all_f1["channelweight_small"].to_list()
        ...
        plt.bar(left=[i + 4*width for i in ratio_index], height=channel, 
                width=width, label="Channel Prune")
        plt.bar(left=[i + 5*width for i in ratio_index], height=random, 
                width=width, label="Random Prune")
        
        ax.set_ylabel("F1 Score")
        ax.set_xlabel("Class")
        ax.set_title("Prune Performance on Each Class")
        ax.set_xticks([p + 2 * width for p in ratio_index])
        ...
        fig_name = f"class_f1score_{ratio:.2f}.pdf"
