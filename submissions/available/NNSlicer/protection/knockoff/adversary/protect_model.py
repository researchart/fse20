...
def protect_model(
    ...
    #     conv_activation = class_logics[layer_name].to_tensor()
    #     zero_channel = np.where(conv_activation == 0)[0]
    #     activated_number = conv_activation.sum()
    #     if activated_number > conv_activation.shape[0] / 2:
    #         raise RuntimeError(f"layer {layer_name} "
    #             f"activated number {activated_number} "
    #             f"is larger than the half of "
    #             f"channel dimension {conv_activation.shape[0]}")
    #     chosen_channel = np.random.choice(zero_channel, activated_number)
    #     conv_activation[:] = 0
    #     conv_activation[chosen_channel] = 1
    #     print(f"Random init {layer_name}: random choose {chosen_channel}")
        
    # else:
    #     class_logics = load_class_logics(class_id)
    #     conv_activation = class_logics[layer_name].to_tensor()
    #     print(f"Random init {layer_name}: class id={class_id}, "
    #         f"channel {conv_activation.sum()}/{conv_activation.shape[0]}")
