...
from knockoff.utils.model import (
    test_step, test_step_single_class, test_step_subset,
    filter_dataset, filter_train_dataset
    )
    ...
    **kwargs
    ...
    for combination_number in [3,4]:
        ...
        log_dir = os.path.join(
            out_path, 
            f"all_combin_{combination_number}",
        )
        ...
        for cnt, class_ids in enumerate(class_id_combination[:max_combination]):
            ...
            log_path = os.path.join(
                log_dir, 
                f"finetune_subset_{class_ids}.csv"
            )
            ...
            # for budget in range(10000, 20000, 5000):
                transferset = transferset_fn(budget=budget)
                # Use only subset to train
                transferset = filter_train_dataset(transferset, class_ids)
                train_fn = partial(
                    train_fn,
                    trainset=transferset,
                )
                for ratio in np.arange(0.5, 0.9, 0.1):
                # for ratio in np.arange(0.6, 1.0, 0.05):
