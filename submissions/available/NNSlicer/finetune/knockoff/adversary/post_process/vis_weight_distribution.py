...
def vis_weight_distribution(
    ...
    for layer_name in layer_names:
        ...
        filename = f"{layer_name}.pdf"
        path = os.path.join(out_path, filename)
        plt.savefig(path)
        plt.clf()
        
def vis_trace_distribution(
    model, trace, out_path,
):
    for layer_name in layer_names:
        weight_cnt = trace[layer_name].flatten()
        
        fig = plt.figure()
        plt.hist(weight_cnt, bins=40, 
                facecolor="blue", edgecolor="black", alpha=0.7)
        
        plt.title("Trace Distribution")
...
if __name__=="__main__":
    ...
    # out_path = "models/adversary/vis_weight_distribution/weight"
    # os.makedirs(out_path, exist_ok=True)
    # model = get_transferred_pytorch_model()
    # vis_weight_distribution(model, out_path)
    
    out_path = "models/adversary/vis_weight_distribution/trace"
    os.makedirs(out_path, exist_ok=True)
    class_trace = load_class_logics(
            "posnegweight",
            "all"
    ...
    model = get_transferred_pytorch_model()
    vis_trace_distribution(model, class_trace, out_path)
    # model = model.cuda()
