  ...
  public class SMTLibProcessTheoremProver : ProverInterface
  {
    ...
    */
    ...
    public override Tuple<int, int> ExtendedCounterexamples(ErrorHandler handler, VCExpr env, int bound, Dictionary<Counterexample,Boolean> IsImpl, int taskID = -1)
    {
        ...
        testBound.Add(90);
        testBound.Add(100);
        testBound.Reverse(); // Large interval first
        var result = Outcome.Undetermined;
                ...
                foreach (VCExpr val in state) {
                    ...
                    if (!solveDuplicateVC.Contains(vcexpr))
                    {
                        ...
                        for (int i = 0; i < testBound.Count; ++i) {
                            ...
                            GeVCExprR = gen.Integer(BigNum.FromString(ParseSimpleVCExpr(GeVCExprR).ToString()));
                            List<VCExpr> GeList = new List<VCExpr>();
                            GeList.Add(exp1);
                            GeList.Add(GeVCExprR);
                            VCExpr GeVCExpr;
                            VCExpr GeVCExpr0 = gen.Function(VCExpressionGenerator.GeOp, GeList);
                            if (lastGeR != null) {
                                List<VCExpr> GeVCExprOrList = new List<VCExpr>();
                                List<VCExpr> GeVCExprOrNotList = new List<VCExpr>();
                                GeVCExprOrNotList.Add(lastGeR);
                                GeVCExprOrList.Add(GeVCExpr0);
                                GeVCExprOrList.Add(gen.Function(VCExpressionGenerator.NotOp, GeVCExprOrNotList));
                                GeVCExpr = gen.Function(VCExpressionGenerator.OrOp, GeVCExprOrList);
                            }
                            else GeVCExpr = GeVCExpr0;
                            lastGeR = GeVCExpr0;
                            ...
                            GeVCExprL = gen.Integer(BigNum.FromString(ParseSimpleVCExpr(GeVCExprL).ToString()));
                            List<VCExpr> LeList = new List<VCExpr>();
                            LeList.Add(exp1);
                            LeList.Add(GeVCExprL);
                            VCExpr LeVCExpr;
                            VCExpr LeVCExpr0 = gen.Function(VCExpressionGenerator.LeOp, LeList);
                            if (lastLeR != null) {
                                List<VCExpr> LeVCExprOrList = new List<VCExpr>();
                                List<VCExpr> LeVCExprOrNotList = new List<VCExpr>();
                                LeVCExprOrNotList.Add(lastLeR);
                                LeVCExprOrList.Add(LeVCExpr0);
                                LeVCExprOrList.Add(gen.Function(VCExpressionGenerator.NotOp, LeVCExprOrNotList));
                                LeVCExpr = gen.Function(VCExpressionGenerator.OrOp, LeVCExprOrList);
                            }
                            else LeVCExpr = LeVCExpr0;
                            lastLeR = LeVCExpr0;
                ...
                while (true)
                {
                    ...
                    else//if unsat 
                    {
                        ...
                        foreach (string s in res)
                        {
                            ...
                            if (exp.Op == VCExpressionGenerator.OrOp) {
                                exp = exp[0] as VCExprNAry;
                            }
