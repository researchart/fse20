...
//            if (argVals.ContainsKey(expr)) return argVals[expr]; // Comment@Xu: This makes some example worse
        ...
        public string outputDataPointWithLabel(dataPoint p, Dictionary<dataPoint, int> labels, bool dup=false) {
            string res = "";
        ...
        public void generateC5Files()
        {
            ...
            foreach (var model in c5samplePointToClassAttr.Keys)
            {
                dupDataFile += outputDataPointWithLabel(model, c5samplePointToClassAttr, true);
            }
