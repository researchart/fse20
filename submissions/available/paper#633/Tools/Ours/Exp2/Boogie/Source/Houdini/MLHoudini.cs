...
namespace Microsoft.Boogie.Houdini {
    ...
    public class MLHoudini
    {
        ...
        // List that contains the ranges for generialization to try 
        ...
        //@Xu: memorialize the bound of generalized counter-example
        // The left is origin data point!
        Dictionary<dataPoint, List<int>> ceLowerBound;
        Dictionary<dataPoint, List<int>> ceUpperBound;
        Dictionary<dataPoint, int> iceBound;
        ...
        public MLHoudini(Program program, string config, string filename, bool useBounds, string learnerDir="./")
        {
            ...
            //@Xu: Initialize ceLowerBound and ceUpperBound
            ceLowerBound = new Dictionary<dataPoint, List<int>>();
            ceUpperBound = new Dictionary<dataPoint, List<int>>();
            iceBound = new Dictionary<dataPoint, int>();
#endif
            if (debug)
            {
...
//                    var typeSet = collector.info_of_cex.Values.Select(x => x.Item1).ToList();
                    //@Xu: if typeSet do not have NEGATIVE or POSTIVE examples we do not try to generalize
//                    if(typeSet.Contains(CounterExampleType.NEGATIVE)||typeSet.Contains(CounterExampleType.POSTIVE))
                        ...
                        if (proverOutcome != ProverInterface.Outcome.Undetermined && proverOutcome != ProverInterface.Outcome.Valid) {
                            var IsImplDictIt = collector.info_of_cex.Select(x =>
                                new KeyValuePair<Counterexample, Boolean>(x.Key,
                                    x.Value.Item1 == CounterExampleType.IMPLICATION));
                            var IsImplDict = new Dictionary<Counterexample, bool>();
                            foreach (var x in IsImplDictIt) {
                                IsImplDict.Add(x.Key,x.Value);
                            }
                            
                            var solverRes = prover.ExtendedCounterexamples(handler, envVC, bound, IsImplDict, -1);
        ...
        public string outputDataPoint(dataPoint p, bool dup = false)
        {
            ...
            foreach (var exFunc in existentialFunctions)
            {
                ...
                {
                    if (dup && ceLowerBound.ContainsKey(p)) {
                        var lList = ceLowerBound[p];
                        var rList = ceUpperBound[p];
                        var pbound = iceBound[p];
                        for (int i = 0; i < attrVals.Count; ++i) {
                            int lbound = lList[i] == Constants.BOUND_VALUE ? -pbound : lList[i];
                            int rbound = rList[i] == Constants.BOUND_VALUE ? pbound : rList[i];
//                            int lbound = lList[i] == Constants.BOUND_VALUE ? -100000 : lList[i];
//                            int rbound = rList[i] == Constants.BOUND_VALUE ? 100000 : rList[i];
                            ret += ",(" + lbound + "," + 
                                   rbound + ")";
                        }
                    }
...
//            if (argVals.ContainsKey(expr)) return argVals[expr]; // Comment@Xu: This makes some example worse
        ...
        public void getIntervalBoundary(dataPoint p, int label, int pbound, List<int> celb, List<int> ceub, int attrVis = 0) {
            if (attrVis == p.value.Count) {
                var dp = new dataPoint(p.functionName, new List<int>(p.value));
                dupc5samplePointToClassAttr[dp] = label;
                return;
            }
            getIntervalBoundary(p, label, pbound, celb, ceub, attrVis + 1);
            if (p.value[attrVis] != celb[attrVis]) {
                var oldRecord = p.value[attrVis];
                p.value[attrVis] = celb[attrVis] == Constants.BOUND_VALUE ? -pbound : celb[attrVis];
                getIntervalBoundary(p, label, pbound, celb, ceub, attrVis + 1);
                p.value[attrVis] = oldRecord;
            }
            if (p.value[attrVis] != ceub[attrVis]) {
                var oldRecord = p.value[attrVis];
                p.value[attrVis] = ceub[attrVis] == Constants.BOUND_VALUE ? pbound : ceub[attrVis];
                getIntervalBoundary(p, label, pbound, celb, ceub,attrVis + 1);
                p.value[attrVis] = oldRecord;
            }
        }
        
        public string outputDataPointWithLabel(dataPoint p, Dictionary<dataPoint, int> labels, bool dup=false, bool findboundary=false) {
            string res = "";
            res += outputDataPoint(p,dup);
            switch (labels[p])
            {
                case 0: res += ",?\n"; break;
                case 1: res += ",true\n"; break;
            ...
            if(findboundary && iceBound.ContainsKey(p))
                getIntervalBoundary(p, labels[p],  iceBound[p], ceLowerBound[p], ceUpperBound[p]);
            return res;
        }
        public void generateC5Files()
        {
            ...
            foreach (var model in c5samplePointToClassAttr.Keys)
            {
                dupDataFile += outputDataPointWithLabel(model, c5samplePointToClassAttr, false, true);
            }
            foreach (var model in dupc5samplePointToClassAttr.Keys)
            {
                if(!c5samplePointToClassAttr.ContainsKey(model))
                    dupDataFile += outputDataPointWithLabel(model, dupc5samplePointToClassAttr, false);
            }
...
//            }
            ...
            using (System.IO.StreamWriter sw = System.IO.File.CreateText(filename + ".ori.data"))
            {
        ...
        class MLHoudiniCounterexampleCollector : VerifierCallback
        {
            ...
            public override void ExtendCounterexample(Counterexample ce, IList<VCExpr> frees, List<string> freeVaribles)
            {
                container.ExtendCounterexample(currImpl, ce, frees, freeVaribles);
            }
        }
        ...
        public void ExtendCounterexample(string impl, Counterexample ce, IList<VCExpr> restr, List<string> freeVaribles, bool recordImplications = true)
        {
            ...
            //@XU: Now matchFuncAndVars save the function's relation with the restricted vars
            //@XU: Now Tuple element: 1: func arg id; 2: func arg name; 3: Ge(1) or Le(-1); 4: comparing number
            Dictionary<string, List<Tuple<int, string, int, int>>> matchFuncAndVars = new Dictionary<string, List<Tuple<int, string, int, int>>>();
            ...
            for (int j = 0; j < funcCalls.Count; ++j) {
                ...
                for (int i = 0; i < func.Args.Count; ++i) {
                    VCExpr v1 = prover.Context.BoogieExprTranslator.Translate(func.Args[i]);
                    for (int k = i+1; k < func.Args.Count; ++k) {
                        VCExpr v2 = prover.Context.BoogieExprTranslator.Translate(func.Args[k]);
                        if (v1 == v2) {
                            return;
                        }
                    }
            ...
            HashSet<Tuple<string, int>> argsIsFree = new HashSet<Tuple<string, int>>();
            for (int j = 0; j < funcCalls.Count; ++j) {
                var funcName = funcCalls[j].Item1;
                var func = funcCalls[j].Item2;
                for (int i = 0; i < func.Args.Count; ++i) {
                    VCExpr ve = prover.Context.BoogieExprTranslator.Translate(func.Args[i]);
                    if (freeVaribles.Contains(ve.ToString())) {
                        argsIsFree.Add(new Tuple<string, int>(funcName, i));
                    }
                }       
            }
            

            foreach (VCExpr expr in restr) {
                VCExprNAry equalExpr = expr as VCExprNAry;
                Debug.Assert(equalExpr != null && equalExpr.Arity == 2);
                int OpValue = equalExpr.Op == VCExpressionGenerator.GeOp ? 1 : -1;
                int CmpValue = ParseSimpleVCExpr(equalExpr[1]);
                    ...
                    for (int i = 0; i < func.Args.Count; ++i) {
                        ...
                        if (ve.ToString() == argument.ToString()) {
                            ...
                            if (!matchFuncAndVars.Keys.Contains(funcCalls[j].Item3)) {
                                matchFuncAndVars.Add(funcCalls[j].Item3, new List<Tuple<int, string, int, int>>());
                            }
                            matchFuncAndVars[funcCalls[j].Item3].Add(Tuple.Create(i, func.Args[i].ToString(), OpValue,CmpValue));
                            break;
                        }
                    }                                                                                                                                                                            
                }
...
//                Debug.Assert(matchFuncAndVars.ContainsKey(absFuncName));
                ...
                List<int> generalizedValsLowerBound = new List<int>();
                List<int> generalizedValsUpperBound = new List<int>();
                int idx = 0;
                foreach (var m in rhs.ElementAt(0).Item2)
                {
                    int mv = GetModelElementIntValue(m);
                    currentVals.Add(mv);
                    if (argsIsFree.Contains(new Tuple<string, int>(funcName, idx))) {
                        generalizedValsLowerBound.Add(Constants.BOUND_VALUE);
                        generalizedValsUpperBound.Add(Constants.BOUND_VALUE);
                    }
                    else {
                        generalizedValsLowerBound.Add(mv);
                        generalizedValsUpperBound.Add(mv);
                    }
                    idx++;
                }
                dataPoint oriPoint = new dataPoint(funcName, currentVals);
                if(matchFuncAndVars.ContainsKey(absFuncName))
                    foreach(var tup in matchFuncAndVars[absFuncName])
                    {
                        if (tup.Item3 == 1) { // Ge
                            if (generalizedValsLowerBound[tup.Item1] == Constants.BOUND_VALUE ||
                                generalizedValsLowerBound[tup.Item1] <= tup.Item4) {
                                generalizedValsLowerBound[tup.Item1] = tup.Item4;
                            }
                        }
                        else { // Le
                            if (generalizedValsUpperBound[tup.Item1] == Constants.BOUND_VALUE ||
                                generalizedValsUpperBound[tup.Item1] >= tup.Item4) {
                                generalizedValsUpperBound[tup.Item1] = tup.Item4;
                            }
                        }
                    }
                if (ceLowerBound.ContainsKey(oriPoint)) return;
                ceLowerBound.Add(oriPoint, generalizedValsLowerBound);
                ceUpperBound.Add(oriPoint, generalizedValsUpperBound);
                iceBound.Add(oriPoint, bound);
