  ...
  public abstract class MutatingVCExprVisitor<Arg>
    ...
    public virtual VCExpr Visit(VCExprNAry node, Arg arg) {
      ...
      while (NAryExprTodoStack.Count > initialStackSize) {
        ...
        //Console.WriteLine("Current sub expr is {0}", subExpr.ToString());
        if (Object.ReferenceEquals(subExpr, CombineResultsMarker)) {
          //
          // assemble a result
          VCExprNAry/*!*/ originalExpr = (VCExprNAry)NAryExprTodoStack.Pop();

          //Console.WriteLine("Assemble a result, the original expr is {0}", originalExpr.ToString());
          ...
          for (int i = op.Arity - 1; i >= 0; --i) {
            ...
            {
                changed = true;
                Console.WriteLine("subexpr {0} changed to {1}", originalExpr[i].ToString(), nextSubExpr.ToString());
            }
      ...
      //Console.WriteLine(NAryExprResultStack.Peek().ToString());
