...
%P_lat12 = ['( (((safe_long_ant \/ safe_lat_ant) \/ stopped_right_lat) R_[',num2str(RHO),', inf)(a_right_lat_min_brake \/(safe_long_ant \/ safe_lat_ant \/ stopped_ego_lat) )) )'];
...
%P_lat22 = ['( ((safe_long_ant \/ safe_lat_ant) R_[',num2str(RHO),', inf) ((stopped_right_lat_ant -> X [] (right_lat_velocity_pos))\/(safe_long_ant \/ safe_lat_ant))  ) )'];
    ...
    psi2 = ['( ( (safe_long_ant \/ safe_lat_ant) Rns_[0,0.5)(a_ego_lat_lt_max_acc /\ a_right_lat_max_acc)) )'];
    r_psi2 = zeros(length(time),1); 
    for ii = 1:length(r_psi1)
        r_psi2(ii) = dp_taliro(psi2,preds_lat,Data_lat(ii:end,:),time(ii:end))>0;
    end

    %psi3 = ['( (((safe_long_ant \/ safe_lat_ant) \/ stopped_ego_lat) Rns_[0.5, inf) a_ego_lat_lt_min_brake) )'];
    psi3 = ['a_ego_lat_lt_max_acc /\ a_right_lat_max_acc'];
    r_psi3 = zeros(length(time),1); 
    for ii = 1:length(r_psi3)
        r_psi3(ii) = dp_taliro(psi3,preds_lat,Data_lat(ii:end,:),time(ii:end))>0;
    end

    psi4 = ['( (((safe_long_ant \/ safe_lat_ant) \/ stopped_right_lat) Rns_[0.5, inf) a_right_lat_min_brake ) )'];
    ...
    psi5 = ['( ((safe_long_ant \/ safe_lat_ant) Rns_[0.5, inf) (stopped_ego_lat_ant -> X [](ego_lat_velocity_neg))  ) )'];
    ...
    psi6 = ['( ((safe_long_ant \/ safe_lat_ant) Rns_[0.5, inf) (stopped_right_lat_ant -> X [] (right_lat_velocity_pos))  ) )'];
