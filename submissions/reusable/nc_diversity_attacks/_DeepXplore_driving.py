# CW + Diversity Regularization on Driving
...
def main():
    ...
    datasets = [results_Driving_light, results_Driving_occl, results_Driving_blackout]
    models = [dave_o] # [fcnet5, fcnet10, conv1dnet, conv2dnet]
    ...
    # regression => classification params
    num_labels = 25
    ...
    real_path = "C:/temp_imgs/Driving/real_dx_Driving/"
    fake_path = "C:/temp_imgs/Driving/fake_dx_Driving/"
    fid_batch_size = 64
    fid_cuda = use_cuda                     

    with open('logs/dx_Driving_error_log_2020.01.27.txt', 'w') as error_log: 
        ...
        for model in models:
            ...
            for dataset in datasets:
                ...
                inputs = torch.transpose(torch.squeeze(torch.tensor(np.stack(dataset['inputs']))), 3, 1).float().to(device)
                targets = torch.tensor(dataset['targets']).float().to(device)
                binned_targets = torch.tensor(convert_to_categorical(dataset['targets'], num_labels)).long().to(device)

                timestamp = str(datetime.datetime.now()).replace(':','.')

                save_file_path = "assets/dx_results_Driving_" + model_name + '_' + transformation + "_2020.01.27.pkl"   
                ...
                # output diversity
                num_classes = num_labels # len(targets.unique())
                ...
                try:
                    ...
                    adversaries = torch.transpose(torch.squeeze(torch.tensor(np.stack(dataset['adversaries']))), 3, 1).float().to(device)

                    # evaluate adversary effectiveness
                    mse, pert_acc, orig_acc = eval_performance_reg2(model, inputs, adversaries, targets, binned_targets, num_labels)
                    # sample_3D_images(model, inputs, adversaries, targets)
                    ...
                    pert_pred = torch.tensor(convert_to_categorical(pert_output.detach().cpu().numpy(), num_labels)).long().view(-1).to(device)
