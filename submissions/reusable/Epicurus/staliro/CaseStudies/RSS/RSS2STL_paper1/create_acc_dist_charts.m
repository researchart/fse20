...
function create_acc_dist_charts(stSignalData, stSignalBounds, preds_long, time)
    ...
    psi3 = ' (safe_long_ant \/ safe_lat_ant) R_[0.5, inf)(a_ego_gt_min_brake /\ a_front_max_brake) ';
    ...
    %psi5 = ' <>_[0,0.1) (safe_long_ant \/ safe_lat_ant)';
    %psi5 = ' (safe_long_ant \/ safe_lat_ant) R_[0.5, inf)(a_ego_gt_min_brake /\ a_front_max_brake) \/ (<>_[0.5,0.55] (safe_long_ant \/ safe_lat_ant)) ';
    psi5 = ' (safe_long_ant \/ safe_lat_ant) R_[0.5, inf)(a_ego_gt_min_brake /\ a_front_max_brake) \/ (true U_[0.5,0.6) (safe_long_ant \/ safe_lat_ant)) ';
    r_psi5 = zeros(length(time),1); 
    for ii = 1:length(r_psi1)
        r_psi5(ii) = dp_taliro(psi5,preds_long,Data_long(ii:end,:),time(ii:end))>0;
    end
    
    %psi6 = ' <>_[0.5,0.55] (safe_long_ant \/ safe_lat_ant)';
    psi6 = ' true U_[0.5,0.6) (safe_long_ant \/ safe_lat_ant)';
    ...
    title5 = '$$ F_{[0,0.1)} (S_{b,f}^{lon} \vee S_{l,r}^{lat}) $$';%'$\varphi^{lon}$ in Lemma 3.3';
    title6 = '$$ F_{[0.5,0.6)} (S_{b,f}^{lon} \vee S_{l,r}^{lat})$$';%'$\varphi^{lon}$ in Lemma 3.3';
