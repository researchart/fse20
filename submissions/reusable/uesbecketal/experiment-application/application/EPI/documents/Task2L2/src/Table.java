...
public class Table {
	...
	public Table Search(Query q) throws Exception {
		...
		if (q.getType() == Query.QueryType.SELECT) {
			...
			if (q.getTablesToMerge().size() == 2) {
				...
				String leftMergeCol = q.getJoinCondition().get(tablesToMerge.get(0).getName());
				String rightMergeCol = q.getJoinCondition().get(tablesToMerge.get(1).getName());
		...
		} else if (q.getType() == Query.QueryType.UPDATE) {
		...
		} else if (q.getType() == Query.QueryType.INSERT) {
...
//			list.addAll(indicesright);
	...
	public Table fulfill(RequestBuilder request) throws Exception {
		Table newT = new Table();
		if (request.getType() == RequestBuilder.QueryType.SELECT) {
		...
		} else if (request.getType() == RequestBuilder.QueryType.UPDATE) {
		...
		} else if (request.getType() == RequestBuilder.QueryType.INSERT) {
	...
	public Table Search(Query q, Table right) throws Exception {
		q.getTablesToMerge().add(this);
		q.getTablesToMerge().add(right);
