...
# instrment every CMP instutions of program
CC=$(pwd)/../../br_pass/src/afl-clang-fast ./configure --disable-shared; make ; cp xmlwf/xmlwf xmlwf_br; rm br_cnt; make clean
# faster version using fork server
#CC=$(pwd)/../../br_fast_pass/src/afl-clang-fast ./configure --disable-shared; make -j ; cp xmlwf/xmlwf xmlwf_br_fast; rm br_cnt; make clean
