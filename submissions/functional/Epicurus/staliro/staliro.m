...
for ii=1:staliro_opt.runs
    ...
    RUNSTATS.new_run();
    if nargout>1
        if (strcmp(staliro_opt.optimization_solver,'IFBT_UR')) &&  isempty(Oldt)
            [getRun, getHistory] = feval('UR_Taliro',InpRange,staliro_opt,Oldt,controlPointNames, categorical,count);
        else
            if (strcmp(staliro_opt.optimization_solver,'IFBT_ART')) &&  isempty(Oldt)
                [getRun, getHistory] = feval('AR_Taliro',InpRange,staliro_opt,Oldt,controlPointNames,categorical,count);
            else
                [getRun, getHistory] = feval(staliro_opt.optimization_solver,InpRange,staliro_opt,Oldt,controlPointNames,categorical,count);
            end
        end
