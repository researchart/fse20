...
namespace VC {
  ...
  public class VCGen : ConditionGeneration {
    ...
    public VCExpr E_GenerateVC(Implementation/*!*/ impl, HashSet<string> constantsAssumed, VCExpr controlFlowVariableExpr, out Dictionary<int, Absy>/*!*/ label2absy,out List<VCExpr> cases /*out Dictionary<Block, VCExpr> block2Expr */,ProverContext proverContext)
    {
        Contract.Requires(impl != null);
        Contract.Requires(constantsAssumed != null);
        Contract.Requires(proverContext != null);
        ...
        label2absy = new Dictionary<int, Absy>();
        //block2Expr = new Dictionary<Block, VCExpr>();
        cases = new List<VCExpr>();
        return E_GenerateVCAux(impl, constantsAssumed, controlFlowVariableExpr, label2absy, cases, proverContext);
    }
    ...
    public VCExpr E_GenerateVCAux(Implementation/*!*/ impl, HashSet<string> constantsAssumed, VCExpr controlFlowVariableExpr, Dictionary<int, Absy>/*!*/ label2absy,/*Dictionary<Block, VCExpr> block2Expr*/ List<VCExpr> cases, ProverContext proverContext)
    {
        Contract.Requires(impl != null);
        Contract.Requires(proverContext != null);
        Contract.Ensures(Contract.Result<VCExpr>() != null);
        ...
        switch (CommandLineOptions.Clo.vcVariety)
        {
            ...
            //case CommandLineOptions.VCVariety.Dag:
                ...
                //else
                //{
                   // vc = E_LetVC(cce.NonNull(impl.Blocks[0]), constantsAssumed, controlFlowVariableExpr, label2absy, block2Expr, proverContext, out assertionCount);
                //}
                //break;
            case CommandLineOptions.VCVariety.DagIterative:
                vc = E_LetVCIterative(impl.Blocks, constantsAssumed, controlFlowVariableExpr, label2absy, cases, proverContext, out assertionCount);
                break;
            //case CommandLineOptions.VCVariety.Doomed:
            //    //vc = P_FlatBlockVC(impl, constantsAssumed, label2absy, false, false, true, proverContext, out assertionCount);
            //    break;
    ...
    public class ErrorReporter : ProverInterface.ErrorHandler {
      ...
      public override void OnModel(IList<string/*!*/>/*!*/ labels, Model model) {
        ...
        List<Block> careBlock = new List<Block>();
        List<Block> careTrace = new List<Block>();

        Counterexample newCounterexample = TraceCounterexample(entryBlock, traceNodes, trace, assumedCmds, model, MvInfo, incarnationOriginMap, context, new Dictionary<TraceLocation, CalleeCounterexampleInfo>(), careBlock);
        ...
        if (careBlock.Count == 1) {
            foreach (var tblock in trace) {
                careTrace.Add(tblock);
                if (tblock == careBlock[0])
                    break;
                
            }
        }
        else if (careBlock.Count >= 2)
        {
            bool start = false;
            foreach (var tblock in trace)
            {
                if (tblock == careBlock[careBlock.Count-2])
                {
                    careTrace.Add(tblock);
                    start = true;
                }
                else if (tblock == careBlock[careBlock.Count - 1])
                {
                    careTrace.Add(tblock);
                    break;
                }
           ...
           // Contract.Assert(false); // can't reach this location
        }

        List<string> careLabels = new List<string>();
        HashSet<string> blockNames = new HashSet<string>();
        careTrace.Iter(v => blockNames.Add(v.Label));
        foreach (var v in careTrace) {
            foreach (var c in v.Cmds) {
                if (c is AssumeCmd || c is AssertCmd) {
                    blockNames.Add(c.ToString());
                }
            }
        }
        ...
        for (int i = 0; i < labels.Count; ++i) {
            string s = labels[i];
            Contract.Assert(s != null);
            Absy absy = Label2Absy(s.Replace("@", "").Replace("+", ""));
            Contract.Assert(absy != null);
            if (blockNames.Contains(absy.ToString()))
            {
                careLabels.Add(s);
            }
        }
        ...
        #endregion
        //callback.OnCounterexample(newCounterexample, labels, careTrace, null);
        callback.OnCounterexample(newCounterexample, careLabels, careTrace, null);
      }

      // now it's this class's method, don't know if we should put it to the parent interface
      public override IList<VCExpr> ExtendCex(Counterexample ce)
      {
          return callback.GetCounterexampleState(ce);
      }

      // I don't think this is a good design pattern
      public override IEnumerable<Counterexample> GetAllCounterexamples() {
          return callback.GetAllCounterexamples();
      }

      public override IList<string> GetCounterexampleLabels(Counterexample ce)
      {
          return callback.GetCounterexampleLabels(ce);
      }

      public override IList<Block> GetCounterExampleBlocks(Counterexample ce)
      {
          return callback.GetCounterexampleBlocks(ce);
      }


      public override void StartExtendCounterexample(Counterexample ce, IList<VCExpr> frees, List<string> freeVar)
      {
         callback.ExtendCounterexample(ce, frees, freeVar);
      }

      public override string SetCounterexamplePathVCMacro(Counterexample ce)
      {
          return callback.SetCurrentCounterexamplePathExpr(ce);
      }
      public override Absy Label2Absy(string label) {
        //Contract.Requires(label != null);
        Contract.Ensures(Contract.Result<Absy>() != null);
    ...
    //currently just consider one while loop in a implementation and one block of each loop part, I don't trace back to the chain, just add one block each class
    public void ConvertCFG2DAG2(Implementation impl,out Dictionary<int ,List<Block>> classeddBlocks, Dictionary<Block,List<Block>> edgesCut = null, int taskID = -1)
    {
      Contract.Requires(impl != null);
      //classeddBlocks = new List<List<Block>>(3);
      //classeddBlocks[0] = new List<Block>();
      //classeddBlocks[1] = new List<Block>();
      //classeddBlocks[2] = new List<Block>();
      classeddBlocks = new Dictionary<int, List<Block>>();
      classeddBlocks.Add(0, new List<Block>());
      classeddBlocks.Add(1, new List<Block>());
      classeddBlocks.Add(2, new List<Block>());
      ...
      foreach (Block header in cce.NonNull( g.Headers))
      {
        ...
        foreach (Block backEdgeNode in cce.NonNull(backEdgeNodes.Keys))
        {
          ...
          #region Add the backedge to clssed blocks
          classeddBlocks[1].Add(backEdgeNode);
          #endregion
...
//      #endregion
    ...
    static Counterexample TraceCounterexample(
                         ...
                         Dictionary<Incarnation, Absy/*!*/>/*!*/ incarnationOriginMap,
                         ProverContext/*!*/ context,
                         Dictionary<TraceLocation/*!*/, CalleeCounterexampleInfo/*!*/>/*!*/ calleeCounterexamples, List<Block> careBlock)
    {
        Contract.Requires(b != null);
        Contract.Requires(traceNodes != null);
        Contract.Requires(trace != null);
        ...
        if (gotoCmd != null)
        {
            foreach (Block bb in cce.NonNull(gotoCmd.labelTargets))
            {
                ...
                {
                    trace.Add(bb);
                    return TraceCounterexample(bb, traceNodes, trace, assumedCmds, errModel, mvInfo, incarnationOriginMap, context, calleeCounterexamples, careBlock);
                }
            }
        }
    ...
    static VCExpr E_LetVC(Block startBlock,
                      HashSet<string> constantsAssumed,
                      VCExpr controlFlowVariableExpr,
                      Dictionary<int, Absy> label2absy,
                      ...
                      out int assertionCount)
    {
        ...
        Hashtable/*<Block, LetVariable!>*/ blockVariables = new Hashtable/*<Block, LetVariable!!>*/();
        List<VCExprLetBinding> bindings = new List<VCExprLetBinding>();
        VCExpr startCorrect = E_LetVC(startBlock, constantsAssumed, controlFlowVariableExpr, label2absy, block2Expr, blockVariables, bindings, proverCtxt, out assertionCount);
        return proverCtxt.ExprGen.Let(bindings, startCorrect);
    }
    ...
    static VCExpr E_LetVCIterative(List<Block> blocks,
                             HashSet<string> constantsAssumed,
                             VCExpr controlFlowVariableExpr,
                             Dictionary<int, Absy> label2absy,
                             //Dictionary<Block, VCExpr> block2Expr,
                             ...
                             bool isPositiveContext = true)
    {
        ...
        List<VCExprLetBinding> posBindings = new List<VCExprLetBinding>();
        List<VCExprLetBinding> negBindings = new List<VCExprLetBinding>();
        List<VCExprLetBinding> impBindings = new List<VCExprLetBinding>();
        Dictionary<Block, VCExprVar> myBlockVariables = new Dictionary<Block, VCExprVar>();
        

        VCExpressionGenerator gen = proverCtxt.ExprGen;
        Contract.Assert(gen != null);

        VCExprVar v1 = gen.Variable("imp_correct", Bpl.Type.Bool), v2 = gen.Variable("neg_correct", Bpl.Type.Bool), v3 = gen.Variable("pos_correct", Bpl.Type.Bool);
        ...
        foreach (Block block in sortedNodes)
        {
            ...
            blockVariables.Add(block, v);
           // block2Expr.Add(block, vc);

            if (block.Label.IndexOf("LoopBody") > 0) {
                impBindings.Add(binding);
               // myBlockVariables.Add(block, v);
            }
            else if (block.Label.IndexOf("LoopDone") > 0) {
                negBindings.Add(binding);
                //myBlockVariables.Add(block, v);
            }
            else if (block.Label.IndexOf("LoopHead") > 0)
            {  //if it is loop head, we split to two path by hand
                VCExpr BodySuccCorrect = VCExpressionGenerator.True, EndSuccCorrect = VCExpressionGenerator.True;
                foreach (Block successor in gotocmd.labelTargets)
                {
                    if (successor.Label.IndexOf("LoopBody") > 0)
                    {
                        BodySuccCorrect = blockVariables[successor];
                    }
                    else if (successor.Label.IndexOf("LoopDone") > 0)
                    {
                        EndSuccCorrect = blockVariables[successor];
                    }
                }

                VCContext context1_ = new VCContext(label2absy, proverCtxt, controlFlowVariableExpr, isPositiveContext);
                VCContext context2_ = new VCContext(label2absy, proverCtxt, controlFlowVariableExpr, isPositiveContext);
                VCExpr vcBody = Wlp.P_Block(block, constantsAssumed, BodySuccCorrect, context1_);
                VCExpr vcEnd = Wlp.P_Block(block, constantsAssumed, EndSuccCorrect, context2_);

                v1 = gen.Variable(block.Label + "_correct", Bpl.Type.Bool);
                v2 = gen.Variable(block.Label + "_correct", Bpl.Type.Bool);
                impBindings.Add(gen.LetBinding(v1, vcBody));
                negBindings.Add(gen.LetBinding(v2, vcEnd));
            }
            else if (gotocmd != null && gotocmd.labelTargets.Count == 1 && gotocmd.labelTargets[0].Label.IndexOf("LoopHead") > 0)
            {
                VCExpr FakeSuccCorrect = VCExpressionGenerator.True;
                VCContext context3_ = new VCContext(label2absy, proverCtxt, controlFlowVariableExpr, isPositiveContext);
                VCExpr vcFake = Wlp.P_Block(block, constantsAssumed, FakeSuccCorrect, context3_);
                //v3 = gen.Variable(block.Label + "_correct", Bpl.Type.Bool);
                posBindings.Add(gen.LetBinding(v, vcFake));
            }
            else {
                posBindings.Add(binding);
            }
        }

        //Block tb0 = new Block(Token.NoToken, "pos", new List<Cmd>(), new ReturnCmd(Token.NoToken));
        //Block tb1 = new Block(Token.NoToken, "imp", new List<Cmd>(), new ReturnCmd(Token.NoToken));
        //Block tb2 = new Block(Token.NoToken, "neg", new List<Cmd>(), new ReturnCmd(Token.NoToken));
        //block2Expr.Add(tb0, proverCtxt.ExprGen.Let(posBindings,v3));
        //block2Expr.Add(tb1, proverCtxt.ExprGen.Let(impBindings, v1));
        //block2Expr.Add(tb2, proverCtxt.ExprGen.Let(negBindings, v2));

        //cases[0] => positive binding
        //cases[1] => implication binding
        //cases[2] => negative binding
        cases.Add(proverCtxt.ExprGen.Let(posBindings, blockVariables[blocks[0]]));
        cases.Add(proverCtxt.ExprGen.Let(impBindings, v1));
        cases.Add(proverCtxt.ExprGen.Let(negBindings, v2));
    ...
    //iterative calls to generate VC
    static VCExpr E_LetVC(Block block,
                    HashSet<string> constantsAssumed,
                    VCExpr controlFlowVariableExpr,
                    Dictionary<int, Absy> label2absy,
                    Dictionary<Block, VCExpr> block2Expr,
        ...
        if (v == null)
        {
            ...
            else
            {
                ...
                foreach (Block successor in gotocmd.labelTargets)
                {
                    Contract.Assert(successor != null);
                    int ac;
                    VCExpr s = E_LetVC(successor, constantsAssumed, controlFlowVariableExpr, label2absy, block2Expr, blockVariables, bindings, proverCtxt, out ac);
                    assertionCount += ac;
                    if (controlFlowVariableExpr != null)
                    {
                        VCExpr controlFlowFunctionAppl = gen.ControlFlowFunctionApplication(controlFlowVariableExpr, gen.Integer(BigNum.FromInt(block.UniqueId)));
